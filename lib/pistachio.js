...
function mustacheReplaceSections(text, opts) {
  return text.replace(mustacheReplaceSections.positive, function(match, name, content) {
    var res = '';
    if (opts.lambdas || (opts.lambda && (opts.lambda.indexOf(name) > -1))) {
      res+='{{@ ("function" !== typeof this['+stringExpression(name)+'])?"":(this['+stringExpression(name)+']('+stringExpression(content).split('}}').join("}'+'}")+')) }}';
      res+='{{{@'+name+' ("function" === typeof this['+stringExpression(name)+'])?"":(this['+stringExpression(name)+']) }}}'+content+'{{{@'+name+'}}}';
    } else {
      res += '{{{@'+name+' this['+stringExpression(name)+'] }}}'+content+'{{{@'+name+'}}}'
    }
    return res;
...
mustacheReplaceSections.positive=(/\{\{\#(\S+?)\}\}((?:\s|\S)*)\{\{\/\1\}\}/g);
mustacheReplaceSections.negative=(/\{\{\^(\S+?)\}\}((?:\s|\S)*)\{\{\/\1\}\}/g);

function mustacheReplaceVariables(text, opts) {
  return text.replace(mustacheReplaceVariables.comment, '').replace(mustacheReplaceVariables.falseTripple,'{{{$1}}}').replace(mustacheReplaceVariables.tripple, function trippleStache(match, name) {
...
mustacheReplaceVariables.falseTripple=(/\{\{\&\s*(\S+?)\}\}/g);
mustacheReplaceVariables.tripple=(/\{\{\{(?=[^\@])\s*(\S+?)\s*\}\}\}/g);
mustacheReplaceVariables.double=(/\{\{(?=[^\@|\>|\{}])\s*(\S+?)\}\}/g);
...
parse.section=(/\{\{\{\@\s*(\S+?)\s+((?:\s|\S)*?)\s*\}\}\}((?:\s|\S)*)\{\{\{\@\s*\1\s*\}\}\}/g);
...
function exprObjString(obj, opts) {
  ...
  return text.map(stripSpace).join('');
}
...
function compile(text, opts) {
  return compileParsed(parse(text, opts));
}
