...
function parse(text, options) {
  ...
  content = content.replace(parse.bangSection, function(match, name, expression, content) {
    expression = expression.replace(/^\s*|\s*$/g,'').replace(/\s*;$/g,'');
    checkExpression(expression, name);
...
var vm=require('vm');
function checkExpression(expression, name) {
  try {
    vm.createScript('('+expression+')', name || 'expression');
  } catch(ex) {
    console.error('WARNING'+(name?('('+name+')'):'')+': ', ex.message);
...
plainSection.toCode= function(ctx) {
  var code=[];
  code.push('(function() {');
  code.push('  var '+ctx.name+'=(this['+stringify(ctx.expression)+']);');
  code.push('  '+ctx.name+'=(("undefined" === typeof '+ctx.name+') || ('+ctx.name+'===false) || ('+ctx.name+'===null))?[]:'+ctx.name+';');
  code.push('  '+ctx.name+'=_isarray_('+ctx.name+')?'+ctx.name+':['+ctx.name+'];');
  code.push('  return _join_(each('+ctx.name+', function('+ctx.name+', _index_) {');
...
bangSection.toCode= function(ctx) {
  var code = [];
  code.push('(function() {');
  code.push('  var '+ctx.name+'=('+ctx.expression+');');
  code.push('  '+ctx.name+'=(("undefined" === typeof '+ctx.name+') || ('+ctx.name+'===false) || ('+ctx.name+'===null))?[]:'+ctx.name+';');
  code.push('  '+ctx.name+'=_isarray_('+ctx.name+')?'+ctx.name+':['+ctx.name+'];');
  code.push('  return _join_(each('+ctx.name+', function('+ctx.name+', _index_) {');
  code.push('    return ('+ctx.parsed.map(function(item) { return item.code }).join('+')+');');
...
partial.toCode= function(ctx) {
  return [ '(', ctx.parsed.map(function(item) { return item.code; }).join('+'), ')' ].join('');
};
...
trippleExpression.toCode= function(ctx) {
  return [ '(this[', stringify(ctx.expression), '])' ].join('');
};
...
plainExpression.toCode= function(ctx) {
  return 'esc(("undefined"===typeof this['+stringify(ctx.expression)+'])?('+ctx.expression+'):(this['+stringify(ctx.expression)+']||""))';
};
...
function compileParsed(tree) {
  ...
  code.push('  function each(a,f) { var i,m=[]; for(i=0;i<a.length;i+=1) { m.push(f.call(a[i],a[i],i,a)) } return m }');
  code.push('  function esc(t) { t=t+""; return t.split("&").join("&amp;").split("<").join("&lt;").split(">").join("&gt;").split(\'"\').join("&quot;") }');
  code.push('  function _isarray_(a) { return (("object"===typeof a) && (null !== a) && (a.constructor === [].constructor)); }');
  code.push('  function _join_(a, c) { c=c||""; return _isarray_(a)?a.join(c):a; }');
  code.push('  var memo={}, root=data;');
  code.push('  return (function() {');
  code.push('    return ('+tree.code+');');
  code.push('  }.call(data).join(""))');
  code.push('})');
  code = code.map(stripSpace).join(lineBreaks?'\n':'');
  code = UglifyJS.parse(code, {
    filename:'pistachio'
  });
  code.figure_out_scope();
  code.compute_char_frequency({});
  code.mangle_names({});
