...
function mustacheReplaceVariables(text, opts) {
  return text.replace(mustacheReplaceVariables.comment, '').replace(mustacheReplaceVariables.falseTripple,'{{{$1}}}').replace(mustacheReplaceVariables.partial, function(match, name) {
    return '[[> '+name+' ]]';
