...
function parse(text, options, callback) {
  ...
  if(options && options.stripTagSpace) {
    text = text.replace(/<script([^>]*?)>([\s|\S]+?)<\/script>/g, function(match, tag, script) {
      var result;
      try {
...
parse.replace.notsection = {
  search: /\{\{^(\S+)\s+(\S+?)\s*\}\}\}([\S|\s]*?)\{\{\/\1\}\}/g,
  replace: function(match, name, expression, content) {
    return '{{#!' + name + ' !(this[' + stringExpression(expression) + ']) }}' + content + '{{/' + name + '}}';
  }
};
