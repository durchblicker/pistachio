...
Content.prototype.code = function() {
  return [
    '\'',
    this.text.split('\\').join('\\\\').split('\'').join('\\\''),
    '\''
  ].join('');
};
...
Section.prototype.code = function() {
  var inside = this.parts.map(function(part) {
    return part.code();
  }).join('+');
  var result = [];
  result.push('(function() {');
  result.push('  var '+this.name+'=('+this.expression+');');
  result.push('  '+this.name+'=(("undefined" === typeof '+this.name+') || ('+this.name+'===false) || ('+this.name+'===null))?[]:'+this.name+';');
  result.push('  '+this.name+'=_isarray_('+this.name+')?'+this.name+':['+this.name+'];');
  result.push('  return _join_(each('+this.name+', function('+this.name+', _index_, _list_) {');
  result.push('    return ('+inside+');');
  result.push('  }));');
  result.push('}.call(this))');
  return result.join('\n');
};
...
Partial.prototype.code = function() {
  var inside = this.parts.map(function(part) {
    return part.code();
  }).join('+');
  return [
...
Document.prototype.code = function() {
  var result = [];
  ...
  result.push('    return '+this.makeCode()+';');
  ...
  result = [ '(', stream.toString().replace(/^\s*(?:\(\s*)|(?:\s*\))?\s*;?\s*$/g,''), ')' ].join('');
  return result;
};
